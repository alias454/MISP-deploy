...
VARIABLES = {
    ...
    "MISP_DB_USER": Option(required=True),
    "MISP_DB_PASSWORD": Option(default=gen_pass(24)),
    "MISP_DB": Option(required=True),
    # Redis
    "REDIS_HOST": Option(required=True),
    "REDIS_PASSWORD": Option(default=gen_pass(32)),
    # Proxy
    "PROXY_HOST": Option(),
    ...
    "MISP_EXTERNAL_BASEURL": Option(required=False),
    "MISP_ORG": Option(required=True),
    "MISP_ADMIN_EMAIL": Option(required=True, validation=check_is_email),
    "MISP_UUID": Option(default=uuid.uuid4()),
    ...
    "GNUPG_PRIVATE_KEY_PASSWORD": Option(default=gen_pass(24)),
    ...
    # Certificate
    "SELF_SIGNED_ENABLED": Option(typ=bool, default="no")
}
...
def generate_crontab_config(variables: dict):
    render_jinja_template("/etc/crontab", variables)
...
def generate_redis_config(enabled: bool, redis_host: str, redis_password: Optional[str] = None):
    # Check for redis config /etc/redis.conf and update password if set
    if redis_password and os.path.exists("/etc/redis.conf"):
        with open("/etc/redis.conf", "r+", encoding="utf-8") as file:
            content = ""
            for line in file:
                if line.startswith("# requirepass"):
                    # Change requirepass foobared
                    content += line.replace("# requirepass foobared", f"requirepass {redis_password}")
                else:
                    content += line
    ...
    if redis_password:
        ...
        redis_path = f"{redis_path}&{urlencode(params)}"
...
def main():
    ...
    try:
        uuid.UUID(str(variables["MISP_UUID"]))
    except TypeError:
    ...
    variables["MISP_UUID"] = str(variables["MISP_UUID"]).lower()
    ...
    generate_crontab_config(variables)
    ...
    generate_redis_config(variables["PHP_SESSIONS_IN_REDIS"], variables["REDIS_HOST"], variables["REDIS_PASSWORD"])
